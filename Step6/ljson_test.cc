...
static void test_parse_objects() {
  {
    ...
    EXPECT_EQ_SIZE_T(0, value->get_objects_size());
  }
  {
    auto value = ljson_value::create();
    EXPECT_EQ_INT(LJSON_PARSE_OK, value->parse(
    ...
    EXPECT_EQ_INT(LJSON_OBJECT, value->get_type());
    EXPECT_EQ_SIZE_T(7, value->get_objects_size());
    EXPECT_EQ_STRING("n", value->get_object_key(0), value->get_object_key_length(0));
    EXPECT_EQ_INT(LJSON_NULL, value->get_object_value(0)->get_type());
    EXPECT_EQ_STRING("f", value->get_object_key(1), value->get_object_key_length(1));
    EXPECT_EQ_INT(LJSON_FALSE, value->get_object_value(1)->get_type());
    EXPECT_EQ_STRING("t", value->get_object_key(2), value->get_object_key_length(2));
    EXPECT_EQ_INT(LJSON_TRUE, value->get_object_value(2)->get_type());
    EXPECT_EQ_STRING("i", value->get_object_key(3), value->get_object_key_length(3));
    EXPECT_EQ_INT(LJSON_NUMBER, value->get_object_value(3)->get_type());
    EXPECT_EQ_DOUBLE(123.0, value->get_object_value(3)->get_number());
    EXPECT_EQ_STRING("s", value->get_object_key(4), value->get_object_key_length(4));
    EXPECT_EQ_INT(LJSON_STRING, value->get_object_value(4)->get_type());
    EXPECT_EQ_STRING("abc", value->get_object_value(4)->get_string(),
    value->get_object_value(4)->get_string_length());
    EXPECT_EQ_STRING("a", value->get_object_key(5), value->get_object_key_length(5));
    EXPECT_EQ_INT(LJSON_ARRAY, value->get_object_value(5)->get_type());
    EXPECT_EQ_SIZE_T(3, value->get_object_value(5)->get_array_size());
    for (size_t i = 0; i < 3; ++i) {
      auto element = value->get_object_value(5)->get_array_element(i);
      EXPECT_EQ_INT(LJSON_NUMBER, element->get_type());
      EXPECT_EQ_DOUBLE(i + 1.0, element->get_number());
    }
    EXPECT_EQ_STRING("o", value->get_object_key(6), value->get_object_key_length(6));
    {
      auto obj = value->get_object_value(6);
      EXPECT_EQ_INT(LJSON_OBJECT, obj->get_type());
      for (size_t i = 0; i < 3; ++i) {
        auto inside = obj->get_object_value(i);
        char expect = '1' + i;
        EXPECT_TRUE(expect == obj->get_object_key(i)[0]);
        EXPECT_EQ_INT(LJSON_NUMBER, inside->get_type());
        EXPECT_EQ_DOUBLE(i + 1.0, inside->get_number());
      }
    }
  }
}
